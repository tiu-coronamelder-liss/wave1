...
attr(data$belbezig,'label')  <- 'Belangrijkste bezigheid'
...
# Create a dataset with only the curent users and never users for all analyses in which these groups are compared. 
...
attr(data_currentvsneveruser$belbezig,'label')  <- 'Belangrijkste bezigheid'
...
attr(data_currentvsneveruserneutral$belbezig,'label')  <- 'Belangrijkste bezigheid'
...
tab.noby <- tableby(~ geslacht + lftdcat + sted + belbezig + oplmet + burgstat + nettocat + woonvorm + Riskgroup_contact, data=data)
summary(tab.noby)
```
...
Om inzicht te krijgen in welke factoren (demografisch, persoonlijk, sociaal, omgevingsfactoren) worden respondenten die momenteel de CoronaMelder gebruiken vergeleken met de respondenten die de CoronaMelder nooit hebben gebruikt. 
```{r, echo=FALSE, results="asis", tab.cap = "Demografische kenmerken naar gebruikersstatus"} 
tab.noby <- tableby(user_status ~ geslacht + lftdcat + sted + belbezig + oplmet + burgstat + nettocat + woonvorm + Riskgroup_contact, data=data_currentvsneveruser)
