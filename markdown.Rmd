...
```{r AdherenceGen, echo=FALSE, fig.width=7, fig.height=2, fig.cap="Intentie tot naleving gedragsregels voor de komende 2 maanden"}
# Dataframe general adherence to measures + replacing answer options
df_Adherence_Gen <- data %>% gather("Adherence_Gen", "Value", contains("Intention_AdherenceGeneralMeasures_handwashing"), contains("Intention_AdherenceGeneralMeasures_distance"), contains("Intention_AdherenceGeneralMeasures_avoidbusyplaces"), contains("Intention_AdherenceGeneralMeasures_quarantaine")) 
df_Adherence_Gen <- mutate(df_Adherence_Gen, Value = factor(Value, levels=rev(levels(data$Intention_AdherenceGeneralMeasures_handwashing))))

# Plot adherence general measures
ggplot(data=df_Adherence_Gen, aes(fill=Value, x=Adherence_Gen)) + theme_bw() + geom_bar(aes(y = (..count..)/sum(..count..)*4)) + scale_y_continuous(breaks=seq(0, 1, 0.1), labels=percent_format(accuracy = 1)) + coord_flip() + labs(x = "", y = "", fill = "") + scale_fill_manual(values=my_palette7) + scale_x_discrete(labels=c("Drukke plekken vermijden", "1,5 meter afstand houden", "Handen regelmatig wassen", "Thuisblijven bij klachten"))  + theme(plot.margin = margin(0, 0, 0, 0), legend.text=element_text(size=8))
...
```{r, echo=FALSE, fig.width=7, fig.height=2, fig.cap="Waargenomen ernst en vatbaarheid - Niet gebruikers"}
...
``` {r, echo=FALSE, fig.width=7, fig.height=2, fig.cap="Intentie tot opvolgen adviezen zonder symptomen"}
# Order in which you would like to have the items appear (first = top, last = bottom) in the horizontal bar chart
...
# Create the plot
ggplot(data=data_graph_longformat, aes(fill=value, x=variable)) + theme_bw() + geom_bar(aes(y = (..count..)/sum(..count..)*length(graph_variablenames))) + scale_y_continuous(breaks=seq(0, 1, 0.1), labels=percent_format(accuracy = 1)) + coord_flip() + labs(x = "", y = "", fill = "") + scale_fill_manual(values=my_palette_nonuse) + theme(plot.margin = margin(0, 0, 0, 0), legend.text=element_text(size=8))
