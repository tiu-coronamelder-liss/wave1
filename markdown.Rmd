...
HBM_PSus_self1
HBM_PSus_self2 (niet gebruikt vooralsnog)
HBM_PSus_other2
HBM_PSev_self1
HBM_PSev_self2 (niet gebruikt vooralsnog)
HBM_PSev_other2


```{r PSus, echo=FALSE, fig.width=7, fig.height=2, fig.cap="Waargenomen ontvankelijkheid voor het Coronavirus"}
...
ggplot(data=df_PSus, aes(fill=value, x=PSus)) + theme_bw() + geom_bar(aes(y = (..count..)/sum(..count..)*2)) + scale_y_continuous(breaks=seq(0, 1, 0.1), labels=percent_format(accuracy = 1)) + coord_flip() + labs(x = "", y = "", fill = "") + scale_fill_manual(values=my_palette7) + scale_x_discrete(labels=c("Als ik zelf besmet raak\n is de kans groot dat\n ik anderen besmet", "Er is een grote kans dat\n ik in de komende 2\n maanden besmet raak")) + theme(plot.margin = margin(0, 0, 0, 0), legend.text=element_text(size=8))


```

```{r PSev, echo=FALSE, fig.width=7, fig.height=2, fig.cap="Waargenomen ernst van het Coronavirus"}
# Dataframe severity + replacing answer options
...
# Plot general adherence
ggplot(data=df_Adherence_Gen, aes(fill=Value, x=Adherence_Gen)) + theme_bw() + geom_bar(aes(y = (..count..)/sum(..count..)*2)) + scale_y_continuous(breaks=seq(0, 1, 0.1), labels=percent_format(accuracy = 1)) + coord_flip() + labs(x = "", y = "", fill = "") + scale_fill_manual(values=my_palette7) + scale_x_discrete(labels=c("Drukke plekken vermijden", "1,5 meter afstand houden", "Handen regelmatig wassen", "Thuisblijven bij klachten")) + theme(plot.margin = margin(0, 0, 0, 0), legend.text=element_text(size=8))
