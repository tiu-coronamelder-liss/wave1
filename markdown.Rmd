...
library(chisq.posthoc.test) # Kan toch weer weg. 
...
attr(data$sted,'label')  <- 'Stedelijkheid woonplaats'
attr(data$belbezig_simplified,'label')  <- 'Belangrijkste bezigheid'
attr(data$nettocat,'label')  <- 'Netto maandinkomen'
attr(data$oplmet,'label')  <- 'Hoogste opleiding met diploma'
attr(data$burgstat,'label')  <- 'Burgerlijke staat'
...
attr(data_currentvsneveruser$sted,'label')  <- 'Stedelijkheid woonplaats'
attr(data_currentvsneveruser$belbezig_simplified,'label')  <- 'Belangrijkste bezigheid'
attr(data_currentvsneveruser$nettocat,'label')  <- 'Netto maandinkomen'
attr(data_currentvsneveruser$oplmet,'label')  <- 'Hoogste opleiding met diploma'
attr(data_currentvsneveruser$burgstat,'label')  <- 'Burgerlijke staat'
...
attr(data_currentvsneveruserneutral$nettocat,'label')  <- 'Netto maandinkomen'
attr(data_currentvsneveruserneutral$oplmet,'label')  <- 'Hoogste opleiding met diploma'
attr(data_currentvsneveruserneutral$burgstat,'label')  <- 'Burgerlijke staat'
...
tab.noby <- tableby(~ geslacht + lftdcat + sted + belbezig_simplified + oplmet + burgstat + nettocat + woonvorm + Riskgroup_contact, data=data)
summary(tab.noby)
```
...
##### Demografische gegevens naar gebruikersstatus
Om inzicht te krijgen in welke factoren (demografisch, persoonlijk, sociaal, omgevingsfactoren) samenhangen met gebruik worden respondenten die momenteel de CoronaMelder gebruiken vergeleken met de respondenten die de CoronaMelder nooit hebben gebruikt. 

Hiervoor wordt voor elke demografische variabele een chi kwadraat toets uitgevoerd. In het geval dat deze toets aangeeft dat de demografische variabele en de gebruikersstatus afhankelijk zijn, dat betekent dat een of meerdere van de categorien verschillen in percentage gebruikers, wordt er paarsgewijze posthoc tests met FDR correctie uitgevoerd om te bepalen welke categorien significant van elkaar verschillen in percentage gebruikers. 
```{r, echo=FALSE, results="asis", tab.cap = "Demografische kenmerken naar gebruikersstatus"} 
tab.noby <- tableby(user_status ~ geslacht + lftdcat + sted + belbezig_simplified + oplmet + burgstat + nettocat + woonvorm + Riskgroup_contact, cat.stats = "countrowpct", data=data_currentvsneveruser) # cat.stats = "countrowpct") for row percentages in stead of column percentages
...
#### Verwachte effectiviteit naar gebruikersstatus 
